...
$(document).ready(() => {
  ...
  (function () {
    ...
    function getData(domainName) {
      ...
      Promise.all(STATUS_CODES.map((status) => {
        const url = `https://${domainName}/api/v2/search/tickets?query="${status}"`;
        return client.request.get(url, options);
      })).then((responses) => {
        ...
        showError('API request(s) failed.');
        console.log('API request(s) failed.', error);
      })
    }
    ...
    function render(responses) {
      ...
      const max = Math.max(...data.map(d => d.total));
      let chartData = updateChartOnData();
      $('.status').click(() => {
        renderStatusOnHTML(chartData);
      });
      $('.ticket').click(() => {
        client.interface.trigger('click', {
          id: 'ticket',
    ...
    function renderStatusOnHTML(chartData) {
      $('#chart').addClass('minimize');
      $('.status').removeClass('active');
      $(this).addClass('active');
      const tickets = chartData[$(this).data('index')].results;
      $('#ticket-list').html(`
          <table>
            <tbody>
              ${tickets.map(function (ticket) {
